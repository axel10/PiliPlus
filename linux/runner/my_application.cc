...
static void my_application_activate(GApplication *application) {
  ...
  const gboolean use_header_bar = [window]() -> gboolean {
    if (g_file_test(
        g_build_filename(g_get_user_data_dir(), "com.example.mypiliplus", "use_ssd", NULL),
        G_FILE_TEST_EXISTS))
      return FALSE;
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "MyPiliPlus");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "MyPiliPlus");
  }
